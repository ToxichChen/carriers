...
async function fetchData(containerId, profile) {
    ...
    if (result === false) {
        ...
        if (typeof (result[0].jobs) !== 'undefined' && result[0].jobs.length >= 1 && await compareStrings(result[0].jobs[0].companyName.toLowerCase().trim(), profileObject.carrier.toLowerCase()) && result[0].jobs[0].jobTitle.toLowerCase().includes(profileObject.work_sphere.toLowerCase())) {
            is_verified = 1;
        }
